...
$(() => {
    ...
    $('#create_new_customer').click(() => {
        ...
        ADD_NEW_CUSTOMER(customerName, customerEnterState, 0, 0, "", 0, 0, 1)
    })
    ...
    $('#search_gutschein_confirm').click(() => {
        ...
        $.ajax({
                ...
                if(response == 0){
                    ...
                    if(parseInt(split[2]) != 0){
                        ...
                        $.ajax({
                            ...
                            error: function(){
                                _redeemedGutschein = false
                                updateAccountActivity("[ERROR] " + _currentUsername + " | Auftragsblatt-Ausbildung | Redeemed Gutschein", LOGTYPE.ERROR)
                            }
                        })
                    }
                }
            ...
            error: function(){
                _redeemedGutschein = false
                updateAccountActivity("[ERROR] " + _currentUsername + " | Auftragsblatt-Ausbildung | Search Gutschein", LOGTYPE.ERROR)
            }
        })
    })
...
function getSearchedCustomer(){
    ...
    // search by number
        ...
        $.ajax({
            ...
            error: function(){
                updateAccountActivity("[ERROR] " + _currentUsername + " | Auftragsblatt-Ausbildung | Search Customer by Number", LOGTYPE.ERROR)
            }
        })
    }
}
...
function sendAuftragSuccess(){
    ...
    $.ajax({
        ...
        error: function(){
            _redeemedGutschein = false
            updateAccountActivity("[ERROR] " + _currentUsername + " | Auftragsblatt-Ausbildung | Add Auftrag", LOGTYPE.ERROR)
        }
    })
}
